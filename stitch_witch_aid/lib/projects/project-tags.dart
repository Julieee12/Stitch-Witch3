...
class ProjectTags extends StatefulWidget {
  ...
  const ProjectTags(this.tags, {super.key, this.refreshPage, });
          ...
          setState(() {
            ...
            if(text == "All"){
              ...
              state.filteredProjects.forEach((project) {
                //if the project has tags and has tag with name
                //searching by name is fine bc tags r hardcoded
                if (project.tags.isNotEmpty && project.tags!
                    .where((tag) => tag.typename == text)
                    .isNotEmpty) {
                  newFilteredProjects.add(project);
                  print("adding project to new list");
                }
              }); //end of foreach
